...
class Finance_model extends CI_Model
{
	...
	public function populateDebt($monthset, $endmonth, $yearspan, $termdata, $chkCreditState)
    {
    	...
    	foreach ($monthset as $monthnum) {
    		if ($monthnum<>0)
    		{
				...
				if ($chkCreditState==0){
				//Οι Ε.Π αποδείξεις συμπεριλαμβάνονται στις οφειλές
				...
				//Οι Ε.Π αποδείξεις ΔΕ συμπεριλαμβάνονται στις οφειλές! Σ' αυτή τη περίπτωση οι οφειλές που υπολογίζονται προφανώς δεν είναι σωστές
				//αλλά μπορούμε να δούμε ποιές αποδείξεις ΔΕΝ εχουν κοπεί για αυτούς που χρεωστάνε! Κάτι τέτοιο είναι χρήσιμο όταν στο τέλος του μήνα
				//ή της περιόδου πρέπει να κόψουμε τις αντίστοιχες αποδείξεις!
					$this->db->query("INSERT INTO `debt`(`reg_id`, `amount`)
						SELECT `registration`.`id`, `month_price` FROM `registration`
						JOIN `term` ON `registration`.`term_id`=`term`.`id`
						WHERE `term`.`active`=1 AND
						((YEAR(`registration`.`start_lessons_dt`)=YEAR(`term`.`start`) AND MONTH(`registration`.`start_lessons_dt`)>=MONTH(`term`.`start`))
						  OR (YEAR(`registration`.`start_lessons_dt`)=YEAR(`term`.`end`) AND MONTH(`registration`.`start_lessons_dt`)<=MONTH(`term`.`end`)))
						AND NOT EXISTS (SELECT * FROM `post_payment` WHERE `post_payment`.`reg_id` = `registration`.`id` AND `post_payment`.`month_num` = ".$this->db->escape($monthnum).")");					

				//Ενημερώνουμε το πεδίο <Μήνας> του πίνακα tbldebt για το μήνα που αναφερόμαστε...     		
				$this->db->query("UPDATE `debt` SET `month_num` = ".$this->db->escape($monthnum)." WHERE `month_num` IS NULL AND NOT EXISTS (SELECT * FROM `post_payment` WHERE `post_payment`.`reg_id` = `debt`.`reg_id` AND `post_payment`.`month_num` = ".$this->db->escape($monthnum).")");					
				}
